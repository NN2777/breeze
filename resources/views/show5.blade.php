...
function generateFlowchart(element) {
  ...
  function processEdge(obj, nextObj) {
    var nodeId = obj.id;
    var nodeNextId = nextObj?.id;

    switch (obj.nodetype) {
      case "Selection":
        var fromSelection = obj.id;
        var toTrue = obj.TrueBranch[0].id;
        edges.push({ from: fromSelection, to: toTrue });
        
        for (let index = 0; index < obj.TrueBranch.length; index++) {
          var current = obj.TrueBranch[index];
          var currentId = obj.TrueBranch[index];
          var currentNextId = obj.TrueBranch[index + 1];
          if (currentNextId) {
            processNode(current);
            processEdge(currentId, currentNextId);
          } else {
            processNode(current);
          }
        }

        var currentIndex = obj.FalseBranch[obj.FalseBranch.length - 1].id + 1;

        var fromTrue = obj.TrueBranch[obj.TrueBranch.length - 1].id;
        edges.push({ from: fromTrue, to: currentIndex });

        var toFalse = obj.FalseBranch[0].id;
        edges.push({ from: fromSelection, to: toFalse });

        for (let index = 0; index < obj.FalseBranch.length; index++) {
          var current = obj.FalseBranch[index];
          var currentId = obj.FalseBranch[index];
          var currentNextId = obj.FalseBranch[index + 1];
          if (currentNextId) {
            processNode(current);
            processEdge(currentId, currentNextId);
          } else {
        ...
        var fromFalse = obj.FalseBranch[obj.FalseBranch.length - 1].id;
        edges.push({ from: fromFalse, to: currentIndex });
  ...
  for (var i = 0; i < element.length; i++) {
    ...
    if(nextObj != null){
      processNode(obj);
      processEdge(obj, nextObj);
      console.log(obj.id, nextObj.id);
    }else {
      processNode(obj);
    }
    // console.log(obj?.id, nextObj?.id)
