...
function generateFlowchart(element) {
  ...
  // Helper function to process a single node based on its type
  function processNode(obj) {
    ...
    switch (obj.nodetype) {
      ...
      case "Selection":
        ...
        processSelection(obj.TrueBranch, obj.FalseBranch, nodeId);
        break;
      case "End":
    ...
    if (nodes.length > 1) {
      var from = nodes[nodes.length - 2].id;
      var to = nodeId;
      edges.push({ from: from, to: to });
    }
  }

  // Helper function to process true and false branches within a selection
  function processSelection(trueBranch, falseBranch, id) {
    var trueNodes = [];
    var falseNodes = [];

    // Process the true branch nodes
    if (trueBranch) {
      var trueBranchStartId = "true_start_" + id;
      nodes.push({ id: trueBranchStartId, shape: "circle", label: "" });
      edges.push({ from: id, to: trueBranchStartId });

      for (var i = 0; i < trueBranch.length; i++) {
        var obj = trueBranch[i];
        var nodeId = obj.id;
        processNode(obj);
        trueNodes.push(nodeId);

        if (i === trueBranch.length - 1) {
          edges.push({ from: nodeId, to: trueBranchStartId });
        }
      }
    }

    // Process the false branch nodes
    if (falseBranch) {
      var falseNode = falseBranch;
      var falseNodeId = falseNode.id;
      processNode(falseNode);
      falseNodes.push(falseNodeId);

      edges.push({ from: id, to: falseNodeId });
    }

    // Connect the last true node to the output node
    if (trueNodes.length > 0) {
      var lastTrueNode = trueNodes[trueNodes.length - 1];
      var outputId = "output_" + id;
      nodes.push({ id: outputId, shape: "parallelogram", label: "" });
      edges.push({ from: lastTrueNode, to: outputId });
    }

    // Connect the last false node to the output node
    if (falseNodes.length > 0) {
      var lastFalseNode = falseNodes[falseNodes.length - 1];
      var outputId = "output_" + id;
      nodes.push({ id: outputId, shape: "parallelogram", label: "" });
      edges.push({ from: lastFalseNode, to: outputId });
    }
  }
  ...
  for (var i = 0; i < edges.length; i++) {
    var edge = edges[i];
    dot += edge.from + ' -> ' + edge.to + ' [label="", fontsize=0];\n';
  }
